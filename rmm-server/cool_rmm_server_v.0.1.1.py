...
def calc_status(rec: dict) -> str:
    ...
    def_ok  = rec.get("defender", {}).get("status_ok", True)
    def_age = rec.get("defender", {}).get("definition_age_days", 0) or 0
    snap    = rec.get("blackpoint_snap", {})
    snap_condition = snap.get("condition", "running")

    if (cpu > 85 or disk > 85 or mem > 90
            or not def_ok or def_age > 3
...
def build_alerts(rec: dict) -> list[dict]:
    ...
    if defender.get("available") and not defender.get("status_ok"):
        alerts.append({"sev": "crit", "device": hostname,
                        "msg": "Windows Defender protection is OFF", "time": "just now"})
    if defender.get("available") and (defender.get("definition_age_days") or 0) > 3:
        alerts.append({"sev": "warn", "device": hostname,
                        "msg": f"Defender definitions {defender['definition_age_days']:.0f}d old",
                        "time": "just now"})

    # Blackpoint Snap alerts — severity follows the condition value
    snap_condition = snap.get("condition")
    if snap_condition and snap_condition != "running":
        SNAP_SEVERITY = {
            "disabled"      : "crit",   # deliberately turned off
            "not_installed" : "crit",   # missing entirely
            "stopped"       : "warn",   # crashed / restarting
            "unknown_state" : "warn",   # SCM running but no process
            "check_error"   : "warn",   # couldn't determine state
        }
        SNAP_MESSAGES = {
    ...
    for dev_id, rec in store.items():
        ...
        devices_out.append({
            ...
            "defender": {
                ...
                "realtime_protection" : defender.get("realtime_protection"),
                "definition_age_days" : defender.get("definition_age_days"),
                "last_quick_scan_days": defender.get("last_quick_scan_days"),
                "status_ok"           : defender.get("status_ok"),
            },
            "blackpoint_snap": {

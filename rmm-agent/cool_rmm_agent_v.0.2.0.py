...
Collects : CPU %, Windows Defender status, on/off (power) status, current user,
           Blackpoint Snap agent status
...
# ─────────────────────────────────────────────────────────────────────────────
SERVER_URL     = "http://YOUR_SERVER_IP:8000/checkin"   # ← change this
...
def get_defender_status() -> dict:
    """
    Query Windows Defender / Microsoft Defender Antivirus status.
    Uses PowerShell Get-MpComputerStatus (available on Win10/11 with Defender).
    Returns real-time protection state, definition age, and quick scan status.
    """
    ps_script = """
    try {
        ...
        [PSCustomObject]@{
            ...
            realtime_protection  = $s.RealTimeProtectionEnabled
            definition_age_days  = [math]::Round($age.TotalDays, 1)
            last_scan            = $s.QuickScanAge
            full_scan_age_days   = $s.FullScanAge
            threat_count         = $s.NISSignatureVersion  # placeholder
            status_ok            = ($s.AntivirusEnabled -and $s.RealTimeProtectionEnabled)
        } | ConvertTo-Json
    } catch {
    ...
    try:
        ...
        return {
            ...
            "realtime_protection" : bool(data.get("realtime_protection")),
            "definition_age_days" : data.get("definition_age_days"),
            "last_quick_scan_days": data.get("last_scan"),
            "last_full_scan_days" : data.get("full_scan_age_days"),
            "status_ok"           : bool(data.get("status_ok")),
        }
    except json.JSONDecodeError:
        return {"available": False, "error": "bad JSON from PowerShell"}
    except Exception as e:

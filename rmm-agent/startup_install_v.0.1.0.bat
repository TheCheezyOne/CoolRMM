...
:CHECK_PYTHON
echo [%TS%] Checking for Python... >> %LOG_FILE%

python --version >nul 2>&1
if %ERRORLEVEL% == 0 (
    for /f "tokens=*" %%V in ('python --version 2^>^&1') do echo [%TS%] Found: %%V >> %LOG_FILE%
    goto CHECK_DEPS
)

:: Python not found — install it silently from the share
echo [%TS%] Python not found. Installing from share... >> %LOG_FILE%

if not exist "%SHARE_PATH%\python-installer.exe" (
    echo [%TS%] ERROR: python-installer.exe not found on share. Cannot continue. >> %LOG_FILE%
    echo [%TS%] Place a Python installer on the share: %SHARE_PATH%\python-installer.exe >> %LOG_FILE%
    goto END
)

"%SHARE_PATH%\python-installer.exe" /quiet InstallAllUsers=1 PrependPath=1 Include_test=0
if %ERRORLEVEL% NEQ 0 (
    echo [%TS%] ERROR: Python install failed [code %ERRORLEVEL%] >> %LOG_FILE%
    goto END
)
echo [%TS%] Python installed successfully >> %LOG_FILE%
...
:CHECK_DEPS
echo [%TS%] Checking pip dependencies... >> %LOG_FILE%

python -c "import psutil, requests" >nul 2>&1
if %ERRORLEVEL% == 0 (
    echo [%TS%] Dependencies OK (psutil, requests) >> %LOG_FILE%
    goto CHECK_VERSION
)

:: One or both missing — install them
echo [%TS%] Installing psutil and requests... >> %LOG_FILE%
python -m pip install --quiet --upgrade psutil requests >> %LOG_FILE% 2>&1

if %ERRORLEVEL% == 0 (
    echo [%TS%] Dependencies installed successfully >> %LOG_FILE%
) else (
    echo [%TS%] WARNING: pip install returned error [code %ERRORLEVEL%] — will attempt to continue >> %LOG_FILE%
)

:: ── STEP 3: Check for version update ────────────────────────────────────────
:CHECK_VERSION
set LOCAL_VER=0
set SHARE_VER=0
